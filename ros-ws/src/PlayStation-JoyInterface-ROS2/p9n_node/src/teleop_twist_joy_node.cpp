...
: rclcpp::Node("teleop_twist_joy_node", options)
{
  ...
  this->linear_max_speed_ =
    this->declare_parameter<double>("linear_speed", 1.38);
  this->angular_max_speed_ =
    this->declare_parameter<double>("angular_speed", 2);

    RCLCPP_INFO(this->get_logger(), "(x,z)=(%f,%f)", linear_max_speed_, angular_max_speed_);
  try {
